...
void insert()
{
	...
	for (int i = 0; i < SIZE; ++i) {
		if (list[i].hp == 0) {
			scanf("%s %d %d %d %d %s", list[i].name, &list[i].hp, &list[i].mp, &list[i].range, &list[i].speed, list[i].position);
			break;
		}
		else if (num == (SIZE - 1)) {
			printf("Data is Full");
...
void deleteall(char position[])
{
	int num = 0;
	for (int i = 0; i < SIZE; ++i) {
		if (strcmp(list[i].position, position) == 0) {
			memcpy(&list[i], &list[SIZE - 1], sizeof(struct Champion));
		}
		else
			num++;
...
void sortbyhp()
{
	...
	for (int i = 0; i < SIZE-1; ++i) {
		...
		memcpy(&temp, &list[max], sizeof(struct Champion));
		memcpy(&list[max], &list[i], sizeof(struct Champion));
...
int main()
{
	...
	while (1) {
		char choice[20], name[15], position[5];

		printf("1. Search [name]\n2. Insert\n3. Delete [name]\n4. DeleteAll [position]\n5. PrintAll\n6. FindMaxHp\n7. SortByHp\n");
		printf("*************************\nEnter a Command: ");
